/**
 * Malta: 6 cities + 4 supplement → 100 places.
...
const existing = JSON.parse(readFileSync(join(ROOT, "data/seeds/malta.json"), "utf8"));
const supplement = JSON.parse(readFileSync(join(ROOT, "data/seeds/malta-landmarks-supplement.json"), "utf8"));
...
const cities = [
  ...existing.cities.filter((c) => BASE_CITIES.has(c.city)),
  ...Object.keys(supplement).map(cityFromSupplement),
];

const output = { ...existing, cities };
writeFileSync(join(ROOT, "data/seeds/malta.json"), JSON.stringify(output, null, 2) + "\n");

const total = cities.reduce((n, c) => n + c.places.length, 0);
console.log(`✓ malta.json: ${total} places, ${cities.length} cities`);
